...
class MSRVTT(Dataset):
    ...
    def __init__(self, visual_features, flow_features, text_features, audio_features,
        ...
        with open(visual_features, 'rb') as f:
            self.visual_features= pickle.load(f, encoding='latin1') 
        
  
        #pickle_in = open(flow_features,'rb')
        #self.flow_features = pickle.load(pickle_in)
        with open(flow_features, 'rb') as f:
            self.flow_features= pickle.load(f, encoding='latin1') 

        #pickle_in = open(audio_features,'rb')
        #self.audio_features = pickle.load(pickle_in)
        with open(audio_features, 'rb') as f:
            self.audio_features= pickle.load(f, encoding='latin1') 

        #pickle_in = open(text_features,'rb')
        #self.text_features = pickle.load(pickle_in)
        with open(text_features, 'rb') as f:
            self.text_features= pickle.load(f, encoding='latin1') 
        ...
        with open(face_features, 'rb') as f:
            self.face_features= pickle.load(f, encoding='latin1')
        ...
        if coco:
            # adding coco data
            self.coco_visual = np.load(coco_visual_path, encoding='latin1')

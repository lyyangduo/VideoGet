...
class LSMDC(Dataset):
    ...
    def __init__(self, clip_path, text_features, audio_features, flow_path, face_path, coco_visual_path='/data/X_train2014_resnet152.npy' ,coco_text_path='/data/w2v_coco_train2014_1.npy', coco=False, max_words=30, video_features_size=2048, text_features_size=300, audio_features_size=128, face_features_size=128, flow_features_size=1024,verbose=False):
        """
        Args:
        """
        ...
        text_sizes = list(map(len,self.text_features)) #Yang add list() to run it through
        #print (len(text_sizes))
        self.text_sizes = np.array(text_sizes)
        #print (self.text_sizes.shape)
        ...
        if coco:
            print ('ADDing COCO dataset')
            # adding coco data
            coco_visual = np.load(coco_visual_path, encoding='latin1')
            coco_text = np.load(coco_text_path, encoding='latin1')
            ...
            text_sizes = list(map(len,self.text_features))  # Yang changed
            #print (text_sizes)
            self.text_sizes = np.array(text_sizes)
            self.text_sizes = self.text_sizes.astype(int)
